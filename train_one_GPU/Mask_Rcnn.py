...
class Mask_Rcnn(nn.Module):
    ...
    def fast_train_data(self, loc, score, anchor, img_size, bboxes, map_HW, masks):
        ...
        target_norm[:, 1:] = target_norm[:, 1:] / cuda(torch.tensor([W, H, W, H], dtype=torch.float32)) * cuda(
            torch.tensor(
                [mask_W, mask_H, mask_W, mask_H], dtype=torch.float32))
    ...
    for epoch in range(epochs):
        ...
        for imgs, bboxes, num_b, num_H, num_W, masks in dataloader:
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/Mask_Rcnn_%d_1.pth' % step)
            if step >= 90010:
                flag = True
                break
        if flag:
            break
    torch.save(model.state_dict(), './models/Mask_Rcnn_final_1.pth')
