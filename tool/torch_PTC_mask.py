...
def bbox2loc(anchor, bbox):
    ...
    return torch.cat([t_yx, t_hw], dim=1), torch.cat(
        [torch.max(anchor[..., :2], bbox[..., :2]), torch.min(anchor[..., 2:4], bbox[..., 2:4])], dim=-1)
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label,
        ...
        if bbox.shape[0] == 0:
            ...
            inter = cuda(torch.zeros((0, 4), dtype=torch.float32))
            inds_box = cuda(torch.tensor([], dtype=torch.int64))
        ...
        loc, inter = bbox2loc(roiP, bbox[inds_box])
