...
from cascade_rcnn.tool.torch_PTC_test import ProposalTargetCreator as ProposalTargetCreator_box
...
class Mask_Rcnn(nn.Module):
    ...
    def get_loss(self, x, bboxes, num_b, H, W, masks):
        ...
        with torch.no_grad():
            ...
            roi = self.loc2bbox(fast_loc, roi[:, 1:])
            roi = self.filter_bboxes(roi, img_size, self.config.roi_min_size[0])
            roi, roi_inds, loc, label, inter, inter_inds, target = self.fast_train_data(roi, bboxes, masks, img_size,
                                                                                        self.loc_std3)
...
def train_dist(model, config, step, x, pre_model_file, model_file=None):
    ...
    #                    (k in model_dic and 'fast_2' not in k and 'fast_3' not in k)}
    ...
    for epoch in range(epochs):
        ...
        for imgs, bboxes, num_b, num_H, num_W, masks in dataloader:
            # if step <= 500:
            #     warm_lr = get_lr(lr, step)
            #     opt.param_groups[0]['lr'] = warm_lr
            #     opt.param_groups[1]['lr'] = warm_lr
            #     opt.param_groups[2]['lr'] = warm_lr * config.bias_lr_factor
            #     if step % 20 == 0:
            #         print('warm_lr', np.round(warm_lr, 6))

            loss = model(imgs, bboxes, num_b, num_H, num_W, masks)
            loss = loss / imgs.shape[0]
            opt.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(train_params, 35.0, norm_type=2)
            opt.step()

            if step % 20 == 0 and local_rank == 0:
                print(datetime.now(), 'loss:%.4f' % (loss.data), step, opt.param_groups[0]['lr'],
                      opt.param_groups[1]['lr'], opt.param_groups[2]['lr'])
                pass
            step += 1
            ...
            if ((step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1) and local_rank == 0:
                torch.save(model.module.state_dict(),
                           './models/Mask_Rcnn_cascade_%dx_%d_1_%d.pth' % (x, step, local_rank))
            if step >= 90010 * x:
                flag = True
                break
        if flag:
    ...
    if local_rank == 0:
        torch.save(model.module.state_dict(),
                   './models/Mask_Rcnn_cascade_%dx_final_1_%d.pth' % (x, local_rank))


def get_lr(base_lr, step, gamma=0.1,
           warmup_factor=1.0 / 3,
           warmup_iters=500, last_epoch=-1):
    if step >= warmup_iters:
        return base_lr
    alpha = last_epoch / warmup_iters
    warmup_factor = warmup_factor * (1 - alpha) + alpha
    start_lr = base_lr * warmup_factor

    return start_lr + step / warmup_iters * (base_lr - start_lr)
